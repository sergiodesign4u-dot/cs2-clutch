...
|---|---|---|
| `D-59`, sharpened by `D-69` | **A public profile of another player, and the product now holds two opposite verdicts on it.** The live feed's avatar has a destination and the map has no node for it. **`7.1` block 6 refuses one outright**, because it would rebuild the trophy shelf that node was created to replace, and **the founder's capture of 21 August 2026 confirms the baseline's profile is exactly that shelf**, an inventory history with four tabs. **Two carriers, opposite answers, one object.** Both surfaces draw the target and print the absence. It is a scope question now rather than a routing one. `5.10` Profile is the account's own view of itself, `D-36`, and a public one is a different surface: what it shows, what an account can hide, and whether it exists at all before row `A3`'s labelling requirement is enforceable at the data layer. **Until it exists the destination is `[?]`** and stage 04 draws the target without a route, marked | Founder, then a node |
