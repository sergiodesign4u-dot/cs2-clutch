...
**`D-92` closed that venue on 23 August 2026 and the entity changed after all, in one word.** The market is us: the prices are ours, taken from Steam, and the market for these items is this product. **So the Skin's "live market price" is not one part with one owner.** On `1.0`, `3.3` and `3.6` it is Steam's figure in EUR with a Steam link; **on `5.1` and `5.3` it is our own price for a copy, in coins, with no route out.** Both readings sit under the one part named above and under the one backlog row `A1`. **The part is left as it is with the split printed**, `0.6` section 3.3 and `0.11` section 2c, because renaming an entity part to fix a backlog row's ambiguity would put the fix in the wrong file.
...
| Global dialog set | 0.4 and 2.1 are already file-level nodes, 6.2 lives inside 6.1 | **Nothing left over.** The set the pack imagines, sign in plus confirm plus pick a place, is three dialogs here and each already has a home. ~~Sign in is a page rather than a dialog because Steam OpenID leaves the site~~ **Overridden by `D-54` on 21 August 2026 and by `D-99` on 25 August 2026, and the type column of both rows now says dialog.** The old argument is kept rather than deleted: leaving the site is what makes the address survive, not what makes the carrier a page. **A node is typed by what a person meets it as, and both are met over the surface they were already on** |
...
| **2.4** | Sign in with Steam | dialog | MVP | `S-B2`. Steam OpenID, a statement of what the product will and will not read, and **the consent gate `D-26` added: two separate checkboxes, terms and the 18+ declaration, with the provider control inert until both are set**. noindex | 2.5, 2.6, then the starter credit into 3.5 |
...
| No | Name | Type | Scope | INCLUDES | Transitions |
|---|---|---|---|---|---|
| **4.1** | Deposit | dialog | MVP | `S-D1`. Coins with the published rate on the screen, `C1` as `D-28` rewrote it, the spend ceiling for a named period `C2`, the withdrawal threshold stated here and never rising `C4`, the responsible play entry. noindex | 2.7, 4.2, 4.3, 4.4, 4.5 |
...
| ~~`D-91`~~ **Closed 23 August 2026 by `D-92`** | ~~**The market venue every settlement is struck against.** `A1` names "a named market venue" and no file names one. **The as-of renders and the route does not**, on `5.3` and on `5.1` alike.~~ **The argument is kept because the answer has to be read against it.** The venue is us: the prices are ours, taken from Steam, and the market for these items is this product. **So both operands of every settlement are ours**, and **the route did not arrive with the venue and cannot**, because a link to our own listing confirms nothing. The as-of still renders and what renders beside it is the relation to Steam | **Closed.** Three narrower debts replace it, below |
| `D-92` | **How far under Steam we set a copy.** Steam is the one number in the chain a stranger can verify and both of ours are derived from it, so **the relation is the whole of what makes a settlement checkable.** Two walked captures read 39 percent and 27 percent, so it is not a constant, and no rule behind it is published. `[?]`, **and it is ours: an undeclared margin rather than an unknown** | **Founder.** Nothing external gates it |
| `D-92` | **What `A1` requires now that its outbound link cannot leave our own product.** `A1` is a pattern of 14 and the link was buying the fact that a person confirms the number somewhere we do not control. **`0.11` section 2c works it out and returns partly satisfied**, one requirement met, one met in specification, one met on three surfaces and broken on two, one met as a route and wrong in its label, one partly, one not met | `0.11`, then the founder for the two that are claims about pricing |
| `D-92` | **Whether the copies are in fact ours at the moment of withdrawal.** The founder's wording was "in principle" and the capture is consistent with a single seller rather than proof of one. **If a third party supplies the copy at withdrawal time, one word changes on every line above and `A1`'s outbound link recovers its original job** | **Founder**, and it is a question of fact about the live platform |
...
| **Entity 3, Skin, on 23 August 2026 by `D-92`**, after all. Its "live market price" part **denotes Steam on the acquisition surfaces and us on the exit surfaces**, one part, two numbers, and the part is left as it is with the split printed | **The node count**, again. 68 map nodes of which 52 are MVP. `D-92` removes no state's subject and promises no destination, so nothing was added and nothing dissolved |
