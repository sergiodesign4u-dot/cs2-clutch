...
flowchart TD
    ...
    Landed -->|no| Resume["Interrupted: the roll was settled before the animation, so the result is waiting on return"]
    Resume --> Outcome
    ...
    Alive -->|no| Gone["Empty: the result is gone or private, with a route into S-A2"]
...
flowchart TD
    ...
    Empty2 --> Cat
    Any -->|yes| Pick(["Continues into flow 1 at the choosing phase of S-C2"])
...
flowchart TD
    ...
    Ceiling --> Dir{"Changed the ceiling, and in which direction?"}
    Dir -->|"lower, or unchanged"| Threshold["The sum required to withdraw is stated here and can never rise, C4"]
    Dir -->|higher| Pending24["Pending: a raise applies 24 hours later. The old ceiling holds until then"]
    Pending24 --> Threshold
    ...
    class Stop dead;
    class Dep,Hit,Ver,Ident,IdWait,IdOk,IdFail,IdAppeal,Amount,Ceiling,Dir,Pending24,Threshold,Pay,Credited,Pending,PayErr neutral;
```
...
flowchart TD
    ...
    NotYet --> Inv
    Elig -->|yes| Start2["S-E2 Withdrawal, with the public clock"]
...
flowchart TD
    ...
    Link --> PF["S-A2 Provably fair, public, no login, H1"]
