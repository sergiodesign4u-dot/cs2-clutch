...
## 5. Nine rules every published number obeys
...
7. **A number is never a score.** No summing across kinds, which is why the header's two money figures are never added, node `0.1`. No streaks, no progress, no celebration, which is the rule `C5` and `T4` attach to limits and which binds this register too: the moment a figure acquires completion mechanics it stops describing and starts encouraging.
