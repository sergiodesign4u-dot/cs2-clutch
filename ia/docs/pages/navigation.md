...
**The toggle carries a menu icon and never a chevron, and stage 04 drew it that way on 20 August 2026 after rendering a chevron first.** The quote is already in section 3 of this node: "the expanded navigation rail **should always open from a menu icon**". **A chevron names a direction; a menu icon names what is behind the control**, and it is the same glyph the mobile header uses for the same job, so one control reads as one control at every width. **The wireframe had invented the chevron against this node's own source**, which is a render diverging from its specification rather than a decision.
...
|---|---|---|
| **Rail collapsed** | Icons with the active indicator, labels gone, tooltips on hover and on focus | Material: collapsed and expanded transform into each other from the menu button. The collapsed rail is never hidden on desktop. **Drawn on 20 August 2026, and it had been specified here since the node was written without any page rendering it.** The rail goes 220px to 64px, the labels go, **the controls do not:** the sound control becomes its glyph and the language keeps its two characters, because a label is a name for something still on screen while a control that disappears is a control that was removed |
