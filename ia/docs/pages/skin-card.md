...
**Amended again the same day by `D-92`, and this one costs the card something rather than closing a hole.** The market the settlement is struck against is named and the answer is us, **so field 12 now holds the Steam price on `3.3` phase 1 and our own price for a copy on `5.1` and `5.3`.** One field name, two numbers, two owners, section 3.3. **It is printed as a contradiction with an owner rather than fixed here**, because both available fixes are moves section 0 forbids: adding a sixteenth field, or renaming a field mid-project.
...
### 3.3 Field 12 holds two different numbers, and `D-92` is what made them different

**Opened here 23 August 2026, read off the drawn screens rather than reasoned from the field set.** Field 12 is "market price, its as-of and its outbound link", one field, one name, five densities.
...
**This node does not resolve it, and the reason is the rule in section 0.** Splitting field 12 into two fields would add a sixteenth to the card, which is exactly the move section 3.2 spent its argument refusing for the settlement. **Renaming it would rename a field mid-project**, which section 0 forbids by name. **So it is printed as a contradiction with an owner rather than fixed by a component file**, and the owner is whoever settles what `A1` means on this product.

**And one thing follows immediately for stage 04 and after.** A density that renders field 12 **must say which of the two it is showing**, because "market price" alone is now ambiguous on the face of the card. `5.1` and `5.3` already do it in words; `3.3` says "on Steam" in its own line and that is the pattern.
