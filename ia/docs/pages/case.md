...
**Half of that hole was answered on 23 August 2026, `D-92`, and the half that was answered is the half nobody had asked about.** The founder named it in passing: **cases are formed at low, medium and high risk.** The band is therefore **an input at the moment a case is built, not a label computed from the finished drop table**, which is a different object from the one `D-24` assumed and the one this node has been specifying against. **What follows from that, and it is a real change rather than a note.** A derived label has a method a reader can rerun on figures printed on the same page, which is the property `0.11` claims for this row and the property that made it recomputable without leaving the screen. **A declared input has no such route**: it is a setting, and a setting is checked by publishing the rule behind it or not at all.

**The thresholds are still `[?]` and the zone still renders as a hole**, so nothing on the screen changes today. **What changed is what the missing thing is.** It was a formula and a set of cut points; it is now a set of cut points and **the rule by which a case is assigned a band when it is built**. `0.11` owns both, and this node records the half rather than closing the row.
...
**Amended 23 August 2026 by `D-92`, and the route survives while the reason for it does not.** The founder named the venue `A1` asks for and the answer is us. **The link drawn on this screen points at Steam and it should stay pointed there**, because Steam is the one number in the chain a stranger can read without us and a link to our own listing would prove nothing.

**What breaks is what the link says it is for.** `cjm-to-be.md` describes it as "an outbound link to buy it on the market instead", and the drawn label reads "Open on the market". **Since `D-92` a copy from us is 39 percent and 27 percent under the Steam listing on the two walked captures**, so a control offered as the cheaper alternative leads to the dearer place while we sell the same copy cheaper. **A concession that costs the reader money is not a concession.**

**The defect predates the decision that exposed it, and by hours.** `D-91` already fixed that the settlement is struck against a purchasable copy and never against the Steam listing, and that the Steam listing is the higher of the two. `D-92` removed the last reading under which that could be overlooked.

**So the route is not in question and its wording is**, and this node does not write the replacement: whether we tell a person the cheaper source is us is a claim about our pricing and belongs to the founder before it belongs to stage 05. The accepted-cost protection above is kept whole because it has to survive whatever the label becomes. Full working and the verdict on `A1`: `numbers.md` section 2c.
...
**Amended 23 August 2026 by `D-92`, and one word of the sentence above stops being true.** "Buys a real copy at whatever it costs on the market that day" reads as an outside cost we absorb. **The copy is ours and the price is ours**, taken from Steam and set under it. So the wording on the drawn screens states the fact rather than the picture: **what we charge for a real copy is a different number, also ours and also set against Steam, and taking an item out settles the difference.** **The gap is still structural and the reason above is unchanged**, because the freeze inside a case is what creates it and a price we set on both sides does not un-freeze anything. **What changes is who the reader should hold responsible for the size of it**, and the honest answer is us.

**And the caveat travels with the wording.** The founder said "in principle", `D-92` section 2d of `numbers.md`. **If the copy is in fact sourced from a third party at withdrawal time, the earlier wording was right and this one is wrong**, which is why the drawn sentence says a copy has to be bought rather than naming who buys from whom. Owner founder.
...
| Part | State |
|---|---|
| **The value** | **`[?]`. It renders as an absence and never as a placeholder.** `0.11` rule 3, missing is a state and never a zero, **and a zero printed here would read as "our values match the market"**, which is the one claim the two walked captures disprove outright. **Amended 23 August 2026 by `D-92`: the value is still `[?]` and it is no longer waiting on anybody.** Both operands are ours, so the figure is arithmetic on two numbers the platform already holds. **It stays absent because we do not hold it in this repository and `CLAUDE.md` forbids inventing one**, and its owner is now a decision rather than a dependency |
| **The method** | **`[?]` and owned by `0.11`**, which is also where the tested return's own hole is owed from. `numbers.md` names what a method would have to say. **Amended by `D-92`: one of the eight things a method has to state closed**, the venue, and **none of the remaining six waits on a third party**. What a method has to answer instead is how the relation to Steam is stated, because Steam is the only input a stranger can check |
| **The cadence** | **`[?]`.** How often it is read is part of the claim, the same way the rolling window is part of the p90's claim. **`D-92` takes the cost out and leaves the claim in:** both sides move under our own hand, so the interval is chosen and not afforded |
...
**A single roll stays horizontal, and that is not an inconsistency.** With nothing beside it to compare against, the wide axis is the one that gives the artwork its room. The axis changes when there is more than one thing to line up, which is the only condition that makes the vertical reel better.
...
**One of three taps to the main job is this gate**, and `sitemap.md` records the cost out loud: the compliance layer eats a third of the budget for the main job. **It is also why the gate carries only the 18+ declaration.** `D-17` put identity verification before funding instead, and if that check sat at this tap the main job would be unreachable inside budget for a first-time visitor. **`D-26` then took that layer out of round 1 altogether**, so the declaration on `2.4` is not the cheap half of a split any more: it is the whole of the age control.
...
**Amended 1 September 2026 by `D-105`, and the amendment is a hole rather than a change of position.** The condition this section states is unchanged: the gate fires at first case interaction and `2.1` owns everything about it. **What changed is that the built prototype turns out never to have wired it to a control this node names.** `gate.md` section 1's own table fires on the open control, the daily free case, the starter credit and "any control that commits the person to a roll". The one live trigger was on the guest sign-in press, **which commits nobody to a roll and which that table excludes**, and the `Open` control on the three account states carried no trigger at all. `D-105` took the wrong wire out and did not add the right one, because `2.1`'s check state does not resolve and putting it in front of `Open` would make `Open` a dead end. **The gate now renders only on its five pinned pages, and where it fires in the prototype is an open question owned by `2.1`.**
...
**Thirteen of these rows are drawn pages in stage 04 and the rest are variations inside them**, `wireframes/_nav.js`: signed in and funded, `3.4`, `3.5`, `3.6`, `3.7`, `D-B` negative, values degraded, since `D-35` the reveal and the outcome at two rolls and at five, and since `D-46` **phase 1 itself at two and at five**.

**And the registry names them the way this matrix does, `D-46`.** Until then each state carried its node code typed into its label as text, so one renderer printed every state as `3.3` and another printed `3.4` through `3.7` for the same eleven pages, **and the flow walker found a state by searching for its code inside a string.** The code is a field now. **A state with no code of its own is a variation of this node and is shown as one**, rather than borrowing a number it does not have. **The two figure conditions were drawn rather than described because only a render answers what they cost:** `D-B` negative is the state where the ranked `Best drops` row of `D-32` stands with no counterweight, and values degraded is the state where the expected value must not be printed from the last numbers we had.

| State | Phase 1 | The trigger | Notes |
|---|---|---|---|
| **Guest** | Full page, every figure. **The stage carries the artwork alone: no lane**, `D-32` | **The trigger reads `Sign in`, `D-105`, and opens `2.4` on this page**. ~~Routes into `2.1`, which is where the journey actually starts.~~ **`2.1` renders nothing on an open market**, its own rule against a flash, so there is no layer between the press and the sign-in dialog | `D-17`, and the numbers are readable behind it. **The label is the act that is available**, not the act the person cannot perform yet. The entry cost stays beside it, because a guest deciding whether this case is worth it needs the price and cannot read it off a button that no longer carries it |
| ~~**Guest, declared**~~ | ~~Full page, no lane~~ | ~~The trigger reads `Sign in with Steam` and routes to `2.4`, then back here~~ | **This row and the one above it were one row from 18 August 2026 and nobody merged them, `D-105`.** `D-26` took the 18+ declaration out of `2.1` and into `2.4`, so there is no declaration for `2.1` to hold and no state for this row to be in. **Kept struck rather than deleted**, because a state that stops existing leaves a hole with a name. `flows.md` flow 1 still holds: Steam refusal and Steam unavailable are `2.4`'s states, and the second returns here so a person who cannot sign in can still read the product |
...
| **Loading the table** | Skeleton with the chances and the values as the thing being waited on. **It read "the free-unit counts" until 21 August 2026**, `D-60` | Not yet armed | Specified here, not a node, per `sitemap.md` |
...
- **H3:** ~~What happens when an item runs out~~ **Removed 21 August 2026, `D-60`**, with the paragraph under it. Nothing replaces it: the H2 above still has two H3s in the copy and the third had no subject left
...
**The third paragraph was removed on 21 August 2026 by `D-60`.** It read: what happens when an item runs out, every item in this case points at a unit we hold, and when the last unit is won the item stays on this page showing zero free units and cannot be rolled. **It is quoted here rather than deleted** because it is the only place the stock promise was ever made to a reader in plain words, and a promise withdrawn silently is worse than one never made.
...
4. **Every figure is crawlable text, none is an image**, `0.11` rule 8. The chances, the values, the RTP and the EV especially: **the pre-login figures on this page are the product's entire pre-login argument**, and an image would remove them from search and from a screen reader in the same stroke.
...
| **The per-case gap figure, its method, its cadence and the date the values were last set** | **Opened 23 August 2026 by `D-91`, section 5.3, and narrowed the same day by `D-92`.** The fifth figure in block 7 has no value, no method and no cadence, and the freeze date behind the case's own values is not published either. **All four render as absences and none as a placeholder**, because a zero here would read as "our values match the market". **What `D-92` changed is the owner column and not the screen:** both sides of the subtraction are ours, nothing waits on a third party, and **a number we set on both sides and decline to publish is a decision rather than a gap** | `0.11`, the published numbers register. **A decision, no longer a dependency** |
| **How far under Steam we set a copy** | **Opened 23 August 2026 by `D-92`.** It is the one figure that would make both of our prices checkable, because Steam is the only number in the chain a stranger can read without us. **Two walked captures read 39 percent and 27 percent, so it is not a constant**, and neither the figure nor the rule behind it is published. **An undeclared margin rather than an unknown**, `numbers.md` section 2c requirement R5 | **Founder** |
| **What the outbound market link on the top item says it is for** | **Opened 23 August 2026, section 3, "The drop table is a comparison table".** The link is drawn and points at Steam, which is right for verification. **Its wording offers a cheaper alternative and Steam is the dearer place** since `D-92`, by 39 and 27 percent on the two captures. The route is not in question; what the route is for is. **The defect predates `D-92` by hours and `D-91` created it** | Founder, because it is a claim about our pricing. Stage 05 writes the string afterwards |
| **Whether the copies are in fact ours at the moment of withdrawal** | **Opened 23 August 2026 by `D-92`, carried here because block 7 and the settlement sentence both rest on it.** The founder said "in principle" and the capture is consistent with a single seller rather than proof of one. **If a third party supplies the copy at withdrawal time, the settlement sentence in section 5.1 reverts and `A1`'s outbound link recovers its original job** | **Founder**, and it is a question of fact about the live platform |
...
| **The two venues for the instance value receipt** | `F1` says two named venues. Which two is not decided anywhere. **Narrowed 23 August 2026 by `D-92` and not closed**, section 8.1: the decision answers the settlement's venue, not this one, **and what it does settle is that neither of the two can be us** | Founder, before stage 04 |
