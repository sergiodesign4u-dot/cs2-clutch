...
- **Real-time figures never announce.** `0.11` states it as a rule and `0.2` repeats it for the footer counters: a live region that fires on every round is a denial of service to a screen reader. **The ticker `0.8` is not a live region.** Neither is the free unit count, the observed rate counter or either money figure.
