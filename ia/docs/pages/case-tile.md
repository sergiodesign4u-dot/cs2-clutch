...
| ~~**Daily case, available today**~~ | **Void since `D-68`, 21 August 2026.** `D-61` had made a daily case its own case rather than a marker; `D-68` took its tile off the shelf entirely, and the ladder `0.15` carries its availability. **No surface renders a daily case as a tile**, so this state has nothing to render on | |
| ~~**Daily case, taken today**~~ | **Void since `D-68`, and the open question it carried is answered rather than dropped.** It asked which field could hold "taken today" once the marker left. **The answer is that no field does, because there is no tile:** the ladder's control states what is available and when. **The reset moment stays `[?]`**, entity 13 in `sitemap.md`, and it is the ladder's now | |
