...
**The ceiling is set at the deposit and changed here, and both are true at once.** `C2` puts the choice at the money decision, where `cjm-to-be.md` says the person is calm and the number is one they typed themselves. This page is where the same ceiling is read and lowered later. **Two surfaces, one value, and `4.1` owns the moment it is first chosen.**
