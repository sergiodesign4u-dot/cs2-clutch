...
| **Related Job 1**, arrive with enough confidence to try, `jtbd.md` Section 2 | job | The whole of blocks B2 to B6 |
...
| **`B1-2`**, the buy-instead advice, pattern of 14 | barrier | `A1`, two prices on one item, block B4 |
| **`B2-1`**, the pre-login information wall | barrier | Every figure on this page renders without an account |
| **`B8-2`**, waiting with no attribution, pattern of 6 | barrier | `A4`, the published median and p90, block B6 |
| **Design principle 1**, `CLAUDE.md`, "trust is the product", as limited by `D-14` | design principle | Block B5, the route into `1.2`, carrying the limit with it |
| **Design principle 3**, `CLAUDE.md`, "odds, cost and expected value are visible and legible" | design principle | `D4` in its pre-login form, block B4 |
...
**Two obligations this node was handed, and both are honoured.** `sitemap.md`, "Free entry gets an obligation rather than a surface", makes `S-A1` carry the starter credit offer as a pre-login element: block B7. `sitemap.md`, "Detailed node map", row 1.0, makes it carry the featured case grid directly on the page rather than behind a mode hub: block B3.
...
| **B2** | **The hero** | The single H1, the proposition in one line, the institutional trust unit `A5`, and one route into the main job | `B1-1`, plus Related Job 1. `A5` also stands on DANGER 1's locked resolution in `jtbd.md` Section 6.2 | barrier and job | Bank T1, Refero District and YouTube |
| **B3** | **Featured case grid** | Component `0.7` tiles: case name, item count, entry cost, live free-unit counter `A2`, daily marker where it applies. Plus one explicit route into the full shelf, `3.1` | Main Job, `jtbd.md` Section 1, and `D-20`. `A2` stands on `B8-1` | job and barrier | Bank T1, Key-Drop **DIFFERENT**, Refero District **TAKE** |
| **B4** | **One case, priced in full** | The worked example. Component `0.6` on the featured case's top item, its live market price with a timestamp and an outbound market link `A1`, its published chance and current value `D2`, the published tested RTP and the expected value at this entry cost `D4`, and the cheaper-on-average statement in our own voice | `B1-2` pattern of 14, `B2-1`, `B7-2`, plus design principle 3 quoted in the row above | barrier and design principle | Bank T1 `A1`; the number set is bank T3, "TAKE the absence as the opportunity" |
| **B5** | **Check any round, with no account** | What the proof covers and what it does not, in one short clause, and the route into `1.2` | Design principle 1 as limited by `D-14`, plus Core Job 1 at `jtbd.md` Section 5 conclusion, which names a public provably fair surface as one of three things no competitor offers pre-login | design principle and job | **Not in the bank.** Named in section 2.5 rather than added silently |
| **B6** | **The exit, priced before the entrance** | `A4` rolling median and p90 from our own logs, `G6` commission-free withdrawal to Steam with the Steam Market fee as its comparison, the payment method marks pre-login, and `C4`'s withdrawal threshold stated here | `B8-2` pattern of 6, `B4-3`, `B4-1`, Related Job 1, Related Job 5 | barrier and job | Bank T1, Clash.gg payment strip **TAKE** |
| **B7** | **The starter credit offer** | `I1`, bounded, with the withdrawal threshold `C4` in the same breath, and the route into `2.4` | Related Job 2, `jtbd.md` Section 2. Bound to `C4`, `D1` and `A1` by `cjm-to-be.md`, answer 1 | job | `sitemap.md` obligation on this node |
| **B8** | **The SEO text block** | One H2 with a real heading tree under it, placed by `0.13` | `0.13`, the structural SEO layer, which `CLAUDE.md` puts inside IA | named requirement, per `0.13`'s own parent rule | Bank T1, all four competitors **TAKE** |
...
**B5 is deliberately short and deliberately not first.** Design principle 1 says trust is the product, and `D-14` says what answers the doubt people actually record is published chance, current value and tested RTP at the moment of spending. Those are B4. The verifier route is the bounded claim, so it follows the unbounded one rather than leading the page.

### 1.2 What is above the fold at 360px, and the defence

**Above the fold: the header `0.1`, block B1, block B2, and the top edge of block B3.**

| Element | Why it is there |
|---|---|
| **The ticker, B1** | It is the only block that answers "do drops actually happen here" without asking anyone to read. `jtbd.md` Section 5 scores Social Job 2 at **3 for The Opener** with the note "ambient context sets mood on arrival", and arrival is exactly this moment. It is also the baseline's own placement, directly under the header, `baseline.md` section 3, and `CLAUDE.md` inherits the live drop ticker by name |
| **The H1, in B2** | `blocks.md` T2 records Hellcase putting its H1 below the fold, under 1,400px of product, and `0.13`'s single H1 rule makes ours the first thing after the shell. A page whose heading is not on the first screen has not said what it is |
| **The institutional trust unit `A5`, in B2** | DANGER 1 in `jtbd.md` Section 6.2 is closed, and its locked design implication is a **hybrid hero**, animation leads with the institutional badge in the same hero unit. The badge is not a separate section further down |
| **The first row of B3, cresting the fold** | The page has to read as product rather than as a marketing page, and the main job's tap 1 is a case tile. Cresting rather than fully visible is the point: it says the page continues into cases |

**What is deliberately not above the fold, each with its reason.** The starter credit, for `B4-2` above. The exit figures, because a median withdrawal time answers a question nobody has asked at second zero. The SEO text, by construction.

**The ticker height is a budget rather than a number.** The baseline runs 120px at 1440px, `baseline.md` section 3. At 360px it may occupy **one visual row and no more**, and it may never push the H1 below the fold. The pixel value is stage 04's; the rule is this node's.

**And the first arrival is worse than this, which is stated rather than discovered at the wireframe.** `0.4` fires on arrival and `2.1` fires at first case interaction, so they never contend, `sitemap.md`, the global sweep. But `0.4` does land on top of this first screen on the very first visit, at the width where it costs most. **Two consequences are fixed here:** the H1 sits at the top of the hero rather than under the `A5` unit, so a dialog covering the lower half of the fold cannot take the page's own name with it; and no route in B2 is the only instance of that route on the page.
...
**Five fields per tile, from `0.7`:** case name, item count, entry cost in real currency, live free-unit counter `A2`, daily marker where it applies. `blocks.md` T2 records that Hellcase and Key-Drop ship name, price and image and nothing else, "which makes a person open the case to learn what is in it, which is the extra step `D1` exists to remove".
...
**Zero-stock is visible, never filtered.** `D1`, and `3.2`'s own rule: the shelf is legibly empty rather than silently short. A featured tile whose free-unit counter is at zero stays in the grid and says so.
...
### 2.4 B4. One case, priced in full
...
|---|---|---|
| The featured case's entry cost | `C1` | One real currency, no coin denomination anywhere. The baseline's coin cannot cross |
...
### 2.5 B5. Check any round, with no account
...
**Home renders no variant of `0.14`.** The canonical round proof block has exactly four variants and none of them is a landing page. B5 is a route and a bounded claim, not a fifth variant, and this sentence exists so that a later stage does not add one.

### 2.6 B6. The exit, priced before the entrance
...
**One figure appears twice on this page and that is by design rather than by accident.** `0.11` reads `A4` on `1.0` before login, on `5.3`, and on `0.2` if the footer's statistics slot 2 is approved. If it is, the median and p90 render in B6 and again in the footer of the same page. **They are one figure with one definition, one source and one as-of, and they may never disagree.** That is exactly what a register is for, and it is written here so that a later stage does not resolve the repetition by deleting the wrong one.

### 2.7 B7. The starter credit offer
...
**So the block states, in the same breath: what the credit is, that it is bounded, and what the withdrawal threshold is.** The threshold is already in B6, and B7 restates it rather than referring to it, because `C4`'s acceptance rule is that no withdrawal ever demands a sum that was not named before the money went in.
...
| **`0.7` Canonical case tile** | Featured variant. Five fields, free-unit counter live, daily marker only if the daily free case is in the featured set | `0.7` |
| **`0.6` Canonical skin item card** | **One instance**, the worked example's top item in B4. See the contradiction below | `0.6` |
...
**Resolved in favour of `numbers.md`, because block B4 cannot exist without it.** A worked example that prints a chance and a current value is a skin card by definition, and forking a second card for one page is exactly what the one canonical component rule forbids. **`0.6` renders on `1.0` in one instance.**
...
| **Hero B2** | First screen: H1, proposition, `A5`, one route | H1 stays and stays single. The `A5` unit **moves below the grid**, because the person has already answered the question it exists for |
| **Grid B3** | Unchanged | Unchanged, and it is now the second thing on the first screen |
| **B4 to B8** | Unchanged | Unchanged |
| **Shell** | Sign in in the header, three rail items, three bar items | Two money figures and the deposit control in the header, four rail items, four bar items |
...
|---|---|---|
| **Balance** | The spendable figure, in real currency, `C1`. **The same value the header shows and never a second computation of it** | `4.1` |
...
**Microcopy is a placeholder with a reserved place, not a text.** Stage 05 writes the strings. What this node fixes is which information has to be in each place, and the one recorded line for B4 is quoted in section 2.4 as stage 05's property rather than as final copy.

---

## 7. Adaptive behaviour

**Base: 360px.** One column throughout. The order is exactly section 1's. The featured grid is **two columns** at this width. The state strip in `1.1` is one row that wraps to two rather than scrolling sideways. Nothing on this page scrolls horizontally, including the grid: a horizontally scrolling shelf hides half the product behind a gesture, which is the defect `3.2` and `D1` exist to prevent at a different level.
...
|---|---|
| **B2 hero** | Splits into two: the H1 and the proposition on one side, the `A5` unit on the other. Whichever of the three candidates leads is section 9's open item |
| **B3 grid** | Four columns. Still one grid, still no category rows |
| **B4 worked example** | Two columns: the `0.6` card on one side, the four figures and the statement on the other |
| **B5 proof route** | Unchanged, one narrow unit. It is short by design |
| **B6 exit** | The figures become one row of cells rather than a stack |
| **B7 offer** | Full width band |
| **B8 SEO text** | **Constrained to a readable measure, never the full width.** It is the one block on the page that is read as prose |
...
| 2 | *(no H2)* | **B2 hero.** Its heading is the page's H1 |
| 3 | `Cases you can open right now` | B3 |
| 4 | `What this case actually costs` | B4 |
| 5 | `Check any round without an account` | B5 |
| 6 | `Getting your skin out` | B6 |
| 7 | `Your first open, without a deposit` | B7 |
| 8 | `What opening a case here involves` | B8, with H3s inside it, listed in block C |

**Two blocks carry no H2 and the reason is written here so the stage 04 check stays mechanical.** `0.13` section 5 makes the H2 list the block order and the check "read the block order, read the H2 list, they match or the node is wrong". The rule with its two exceptions: **every content block carries an H2 in order; the hero does not, because its heading is the H1; the ticker does not, because it is a labelled region rather than a section.**

### C. SEO text, finished

Rendered in block B8, under the H2 `What opening a case here involves`.

> A case is a fixed set of CS2 skins with a published chance on each one. You pay the entry cost, one roll decides the item, and the item moves to your Steam inventory when you withdraw it. Nothing here is priced in a coin or a token: every figure on this page is in real currency, so what you read is what it costs.
>
...
5. **Every route is a crawlable `<a href>`**: every grid tile into `/cases/<slug>`, every ticker tile into `/r/<id>`, the shelf route into `/cases`, B5 into `/provably-fair`, B7 into `/signin`. `0.13` section 8: a destination reachable only by script does not exist for a crawler and often not for a keyboard either.
...
9. **The meta description has a source on the page**, `0.13` section 10 item 5: every claim in it is rendered in B4 and B6.
...
**And what belongs elsewhere.** The words in any string on this page: stage 05, which syncs the voice-aligned version back into block C. How the grid, the figures and the hero look: stages 06 and 07. Whether `D-C` returns yes at all, which decides whether half of B4 and part of B6 are drawn: not a design question, `0.11` section 1.
