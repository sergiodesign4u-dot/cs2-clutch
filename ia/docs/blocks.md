...
| Key-Drop, Hellcase | Event banner with a countdown | **TAKE by `D-25`, 18 August 2026. Recorded DIFFERENT and reversed by founder decision** | **None in the three legal classes** | MVP | The original reason stands and is printed rather than withdrawn: a countdown is urgency attached to spending. It ships as block B2 of node `1.0` with six rules bounding it, section 2.2, and **its empty state is that it does not render at all** |
...
| Hellcase | Filter control with an **active count badge** | **TAKE** | MVP Core Job 1. **It was `D1`, the stock-backed table barrier, until 21 August 2026**, and `D-60` took that parent, `catalogue.md` block 2 | MVP | Same control. Ours also has to survive `3.2`, and their filter has no visible empty state |
...
| Hellcase, Key-Drop | Case tile fields: **name, price, image**, and on Hellcase a TOP badge and one bare number | **DIFFERENT** | `0.7` canonical case tile, parent `D1` and `D2` | MVP | **Ours carries four fields after `D-24`: artwork as the largest element, name, entry cost, risk band, with the stock marker and the daily marker appearing only when each applies.** Their tile makes a person open the case to learn what is in it, which is the extra step `D1` exists to remove |
...
| Own research | One real currency, **no coin denomination** | **TAKE**, then **reversed on 19 August 2026 by `D-28`** | `C1` | MVP | Every competitor here denominates in Gems, coins or tokens. Clash.gg prices in Gems, Key-Drop in local currency, Hellcase in a bare number. The reason this was a TAKE still stands: a second currency hides the price. **What `D-28` changed is the mechanism, not the finding**: the product denominates in coins like the category, and publishes the rate at the moment of spending, which no platform in this bank does |
...
**Nodes:** `2.1` Geo gate with `2.2`, `2.4` Sign in with its consent gate plus `2.5` and `2.6`, `0.4` Cookie consent. **`2.3` dissolved by `D-26`, 18 August 2026.**
