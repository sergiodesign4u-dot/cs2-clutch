...
|---|---|---|
| Language | One, English | One IA node is one page. Microcopy is strings, not keys. Voice and glossary run once. **The switcher is drawn, `D-41`: the carrier is inherited and only the dead items are deferred, so it opens one live option and names what is absent, and the product carries no hreflang until a translation exists.** |
