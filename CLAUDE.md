...
- **Inherited, deliberately close to identical:** navigation model, page inventory, page composition, flows, the live drop ticker, the placement of the money and account actions. **Mode switchers are inherited as patterns and deferred as shipments, all of them, and the trigger is the second mode.** The baseline runs two: a left icon rail of nine destinations and a row of four game mode cards on Home. Exactly one destination in each is in round 1, and a switcher with dead items advertises absence on every screen it sits on. Home therefore carries the case grid directly, which is also what keeps the main job at three taps. `docs/decisions.md` D-19 and D-20.
