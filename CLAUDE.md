...
The rule of the course still holds and outranks the resemblance target: a reference is an input, never an output. No screen is carried over whole. Where the inherited structure loses to research, research wins and the divergence is named out loud in the IA node. **And it is named in one fixed place: every node carries a baseline row, what the live product does, what we keep, what we change and why.** An unnamed divergence is how a redesign becomes a different product without anyone deciding to.
...
**Round 1:** home, case opening, registration and account, deposit, skin withdrawal to Steam, provably fair page with verification tool, geo block layer, responsible play page, public result page, roll history, profile, settings. Twelve surfaces, and the list has moved three times by founder decision: on 11 August responsible play was restored after the CJM stage found it named in the canvas and dropped from both scope lists with no verdict, and the public result page was added because row `F4` puts a shared object into the world and nothing said what that object was; on 20 August the last three arrived together because **the account menu may not promise a destination the map does not hold and the founder closed that gap on the map's side**, two of them with no parent in the three legal classes and settings with its round 1 contents still `[?]`. `docs/decisions.md` D-20 and D-36.

**LATER:** case battles, gunfights, upgrades, in-platform exchange, giveaways, leaderboards, referral programme, rakeback. **In-platform exchange is trading a skin for another skin and it stays LATER. Selling a skin back for coins is a different capability, it is in round 1 since `D-38`, and it has no backlog row at all:** the outcome screen carries it with that empty parent printed, and section 8.3 of node `3.3` keeps the argument against it intact rather than deleted.

The MVP core is refined at capability level exactly once, in `research/docs/cjm-to-be.md`: 40 MVP rows over 39 distinct capabilities, `D-24` having added the risk band and the favourite, grouped by the surface each one lives on. No other file holds a competing list. Stage 03a marked every screen and 03b marks every block, both inheriting that mark rather than deriving it a second time. `MVP` means a documented As-Is barrier recurs without the row, not merely that the mechanics still complete. An MVP that was never cut is not an MVP.

Marks travel with that backlog and are carried, not smoothed: the observed rate counter is conditional on decision `D-B`, the blocked-market list is `[?]` until re-verified against current law, the conversion cost of the age gate has no target, and one capability sits in MVP with no parent in the three legal classes at all, the daily free case, placed there by founder decision and carrying that cost in the open. Reasoning: `docs/decisions.md` D-15.
...
Seven clusters by intent, not by site section: decide whether this place is real, get through the door, choose what to open and open it, put money in, take out what I earned, keep myself in check, tell someone. Twelve MVP screens against thirteen LATER when the base layer closed and fifteen since `D-36`, expanded by 03b into 68 numbered nodes of which 52 are MVP, `D-26` having dissolved one and sent three to LATER and `D-36` having added three so the account menu's rows have destinations, a count that grows when a carrier promises a destination the map does not hold or when a structure turns out to be read by several nodes, listed once in `ia/docs/sitemap.md` and never derived a second time. Routes, states and dead ends are in `ia/docs/flows.md`.

**Main flow:** home, case screen, sign in with its consent gate, open, outcome. **The rail carries one destination, Cases, in both states since `D-40`**, with the ambient controls and the social links at its foot. **Provably fair and Responsible play are carried by the footer**, which holds each in its own column, and **My items is the first row of the account menu**, so nothing is lost and no carrier holds another's kind. **On mobile a bottom bar carries a shortcut subset of it, Home plus Cases, in both states**, and it is below Material's floor of three in both: `D-29` printed that for one state and `D-40` widened it rather than reinterpreting the quote. The header carries the money as **two figures, balance and value of items held, both in the coin and never summed and never a score**, and money is never a bar item in either state. **The coin ships pegged and the peg is printed wherever money is spent, `D-28`:** a coin whose rate is not published is the abstraction `B7-1` describes, and that reversal of `C1` is a founder decision carrying a pattern of 7 as its cost. **Depth to the main job is three taps for a first visit and two once an account exists**, and the third is the geo layer where it fires: `2.1` is the geo gate alone since `D-26` moved the 18+ declaration into sign in. `docs/decisions.md` D-17, D-19, D-21, D-22 and D-26.
...
**Quality of research output.** Cite the source of every fact, and cite by section name rather than by line any file a later step may still edit, this one and the research sources above all: a line citation into a moving file rots silently and still reads as sourced. Never invent a number: unknown is `[?]`. Diverge then converge: several options, then the chosen one and why the others were dropped. Public and pre-login pages only, never log in. **One exception, and only one: our own baseline. The founder captures the signed-in surfaces and files dated screenshots in `research/screens/`, and from then on they are a source in the same class as a walked page, cited by shot and date.** A screenshot with no date is model memory with a picture attached. A fact about a competitor comes from a page opened in this session, not from model memory; taken from memory or from an undated article it is `[?]`, not a fact.
