...
**Quality of research output.** Cite the source of every fact, and cite this file by section name, never by line: every stage edits it, so a line citation into it rots silently and still reads as sourced. Never invent a number: unknown is `[?]`. Diverge then converge: several options, then the chosen one and why the others were dropped. Public and pre-login pages only, never log in. A fact about a competitor comes from a page opened in this session, not from model memory; taken from memory or from an undated article it is `[?]`, not a fact.
