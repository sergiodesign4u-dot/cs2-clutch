...
- **Step in the product.** Funds the account, paying in real money and receiving coins at a published rate since `D-28`, and in the same step sets a spend ceiling for a named period.
...
- **Capabilities.** **One coin denomination throughout, pegged and published**, `D-28`, so a payout can be converted back to a price by the person reading it rather than only by us. **Limit chosen at deposit**: the amount just typed is pre-filled as a ceiling with a period selector, the deposit cannot be submitted until the user accepts or changes it, lowering it applies immediately and raising it applies 24 hours later. When the ceiling is reached, deposits stop while opening from existing balance and withdrawal stay fully open. **The first clause was overridden on 27 August 2026 by `D-103`: the limit is set on the responsible play page and the deposit states the figure in force. Everything after it still holds, and the paragraphs below are kept as the argument that was overridden rather than deleted.**
...
**What `D-103` took, and it took the part this step chose over two alternatives.** The argument above is that the deposit is the one moment the person is calm and the number is one they typed themselves, and both dropped candidates were dropped for being worse moments. **A boundary that has to be sought out on a settings page is a boundary most people never set**, and that is what the two dropped candidates were being compared against. The founder read the drawn screen, could not name the control, and moved it. **The record carries the cost rather than the argument being rewritten to fit it.**
...
**Diverge to converge on `B7-1`.** Three candidates. **Chosen: instance value receipt.** The pain is not that a number was optimistic, it is that the number described a different object than the one received, a name instead of an instance. The two rivals both leave that substitution intact. **Dropped as the primary: sell price everywhere**, which fixes the unit but not the number, although its load-bearing rule travels with the pick as a hard dependency: the receipt must be denominated in a unit the reader can price, and a receipt in coins fails that test unless the peg is on the receipt. **`D-28` chose coins on 19 August 2026 and the dependency is what survives: the peg travels with the receipt.** **Dropped: a published real payout per case**, expressing return per dollar at real prices, because it fires before the reveal at the one moment this persona reads nothing, and it pays the highest margin exposure of the three for a gain the Opener will not register. **Restored by the step 9 critique as backlog row D4, on different grounds**: not as an answer to `B7-1`, where this reasoning still holds, but as a disclosure that design principle 3 at `CLAUDE.md` and the acquisition decision at `aarrr.md:67` each require independently of whether the Opener reads it. **Rejected before scoring: a binding buyback at the displayed value**, because it adds a payout rail round 1 does not have; round 1 has exactly one exit, withdrawal to Steam.
...
**Reconciled with design principle 1, because the step 9 critique read this as a demotion.** `CLAUDE.md` states that provable fairness is the brand, not a footnote, and nothing here contradicts it: the product ships three provable-fairness surfaces, a public page with a working verifier, a hash visible at the spin trigger and a post-reveal link, which is more of it and earlier than any platform in the benchmark set. What is demoted is not the feature, it is one claim about the feature, that it answers `B7-2`. Principle 1 says provable fairness is the brand; it does not say provable fairness proves the weight table, and this map says it does not. If that distinction needs to live in `CLAUDE.md` itself, step 11 owns the wording.
...
**Every row names a parent, and there are exactly three legal parent classes**: a barrier code from `cjm-as-is.md`, a job from `jtbd.md`, or a named compliance constraint or design principle from `CLAUDE.md`. The third class was added by the step 9 critique, which found B4 and C5 already standing on it while the rule admitted only the first two. It is enumerated rather than open: only the five compliance constraints in `CLAUDE.md` and the five design principles there qualify, and a row using it must quote which one. A capability with no parent in one of the three classes is not in this table, it is in the cut list below. **Two exceptions exist and both are named rather than assumed.** An item already locked into `LATER` by the round 1 scope decision cannot be cut by this stage even when it has no parent, so it is carried with the parent cell reading `None` and a standing requirement to acquire one before its own round opens. And from 11 August 2026 one MVP row has no parent either: I2, the daily free case, placed in round 1 by founder decision. A founder can overrule this rule and did. What the rule still gets is the row saying so in its own parent cell, so that the exception is countable rather than invisible.
...
Eight rows do not clear even that test on their own and every one of them says so in its own cell rather than hiding behind the label: F4, which the referral loop needs and the path does not; D3, which does not do its job until N grows; A3 and G6, which stand on a job with no barrier behind them; E4, F3 and H1, which are MVP by locked round 1 scope and by founder decision rather than by any persona score; and I2, added on 11 August 2026, which has no parent in any of the three legal classes. The earlier text claimed one exception and had seven, and the critique was right to call that a double standard, since the daily bonus had been cut on a criterion three of these rows also fail. The eighth is that same daily bonus, returned by founder decision, and it is counted here rather than treated as a special case outside the count.
...
**Forty MVP rows, thirty nine distinct capabilities** since `D-24` added `D5` and `D6` on 18 August 2026: A4 and G3 are one feature on two surfaces. Seven of the rows are rules or architecture rather than something anyone draws. The counts moved from thirty seven and thirty six on 11 August 2026, when the founder returned the daily free case to round 1 as I2.

Eight groups cover the eight round 1 surfaces, and the mapping is not one to one, which the step 9 critique caught being stated as if it were. Group 2 covers two round 1 items at once, registration and account together with the age gate and geo block layer. Groups 4, 5 and 6 are three phases of the single case opening screen. Group 8 holds the provably fair page plus two things that are not surfaces at all, I1 the starter credit and I2 the daily free case, both named as components rather than counted as screens. The eighth surface, the responsible play page, has no group of its own: its capabilities are C2 and C5, and they live where the money decision is made, at the deposit. A group is a place rows were written down, not a claim about where the interface puts them.
...
| B3. Age gate before any case interaction. Method open, see `D-A` | Compliance constraint, `CLAUDE.md`, "age verification before any case interaction", plus `B8-4` | MVP | Compliance, not conversion. The conversion cost of the gate is measured separately, target `[?]` |
| B4. Geo block with a cited legal ground per blocked market, and the staged regional rollout that same ground implies | Compliance constraint, `CLAUDE.md`, "geo blocking informed by cited legal research" and "staged regional rollout resting on cited law", sourced at `research.md` section 7 | MVP | Every blocked market carries a row with a citation, and no market opens without one. Inherited starting list, carried `[?]` until re-verified against current law: Belgium, Netherlands, Germany, UK, Washington, Nevada, `lean-ux-canvas.md:55` |
...
| B7. Account and inventory: what the user holds, each item carrying its F1 receipt, plus the account state, verified and withdrawals open | `B7-1`, Related Job 5 `jtbd.md:69` | MVP | F2 has nowhere to persist without this row, and `CLAUDE.md` lists account as a round 1 item |

### 3. Deposit

| Feature | Parent | Priority | Success signal |
|---|---|---|---|
| C1. **One coin denomination throughout, pegged, and the peg printed wherever money is spent.** Reversed on 19 August 2026 by `D-28`, which prints the cost in full: this row was the whole structural answer to `B7-1`, a pattern of 7, and the founder took the live product's denomination instead. **The peg is what is left of the answer and it is not optional** | `B7-1`, pattern of 7, **now only partly answered** | MVP, architecture | No number renders anywhere without its unit, and no coin figure renders anywhere without the peg beside it at the moment of spending. Time figures and ratios do not become coins: `A4`, `G3` and `D4` stay minutes and percentages |
| C2. **Deposit limit, and its moment moved on 27 August 2026 by `D-103`.** The row read "spend ceiling chosen at deposit": the amount just typed pre-filled as a ceiling, blocking the payment until it was accepted or changed. **The founder moved the setting to the responsible play page**, node `6.1`, where the other three boundaries already live, and the deposit surface states the figure in force with a route rather than asking for it. **What the brake does is unchanged. When it is set is not** | `B7-4`, pattern of 12, **and the answer is now weaker at the moment the pattern fires**, which `D-103` prints in full | MVP | Share of accounts carrying a limit at all, and share of sessions stopped by it. **The old signal, share of deposits where the pre-filled ceiling is accepted unchanged, has no subject any more.** Targets `[?]` |
...
| C5. Session limit, self exclusion, cool down | `B7-4`, compliance constraint `CLAUDE.md`, "responsible play tooling" | MVP | Their surface is the responsible play page named at `lean-ux-canvas.md:54`. That page was dropped from the round 1 list with no verdict recorded anywhere, this step found the silence, and the founder restored the page to round 1 on 11 August 2026. C5 now has a locked home rather than a scope question |
...
| D4. Published tested RTP for the case, and the expected value at this entry cost, on the case page and before login | Design principle 3, `CLAUDE.md`, "odds, cost and expected value are visible and legible", plus `B1-2` | MVP | `aarrr.md:67` requires RTP published from day one and `aarrr.md:205` makes matching or beating the tested figures in the category, Hellcase 82.1 percent and DatDrop 80.6 percent, both a trust position and a regulatory hedge. `jtbd.md:180` names published RTP as one of three things no competitor surfaces pre-login |

| D5. **Risk band per case**, three named values computed from the drop table, on the tile and as the published figure it comes from on the case page | **Design principle 3 in `CLAUDE.md`, quoted:** where money is about to be spent, odds, cost and expected value are visible and legible. Added by founder decision `D-24`, 18 August 2026 | MVP | Recomputable by a reader from the chances and values printed on the same screen, the same route `D4` has. **Thresholds `[?]`, and the band does not render until they are set** |
| D6. **Favourite a case:** a per-account mark with a public count | **None in the three legal classes.** In by founder decision `D-24`, which also amends `0.11` rule 7 to admit one public count. **Orphan, printed** | MVP | The person's own press moves the count by one in front of them. **Never a rank, never a default sort, never a most-popular strip, and never beside the open control** |
### 5. The open
...
| I2. Daily free case | Founder Decision 3, `jtbd.md:198`, plus the MVP product decision at `aarrr.md:172`. **No parent in the three legal classes** | MVP by founder decision, 11 August 2026. The only row on this map with no barrier, no job and no named constraint behind it, and it is printed here rather than dressed in a borrowed parent | Retention OMTM, D7 retention rate, `aarrr.md:167`, target `[?]` 35 percent and marked a hypothesis at its source. **It also runs against the residual risk named in answer 1:** a daily free open teaches the same "opening is free" impression the starter credit does, on a repeating schedule rather than once |
...
| ~~Daily free case~~ | Founder Decision 3 at `jtbd.md:198`, plus the MVP product decision at `aarrr.md:172` | **Moved out of LATER on 11 August 2026.** It sat here provisionally for one step, the founder settled it, and it is now row I2 in group 8 with its missing parent stated there. The row is kept struck through rather than deleted, because the reason it was ever in LATER is the reason I2 carries a warning |
...
| "Responsible play page", `lean-ux-canvas.md:54` | C2 and C5, and the page itself is restored to round 1 by founder decision on 11 August 2026 after this step found it dropped with no verdict; see below |
...
- The daily free case was the one disagreement where `CLAUDE.md` did not obviously win, and it is now closed. `aarrr.md:172` is an analyst decision and loses to scope as the others do, but `jtbd.md:198` is founder Decision 3 and says "MVP free-to-play offer is starter credit + daily free case". A founder decision is not a superseded canvas cell. **Settled by the founder on 11 August 2026: it enters round 1 as I2.** What does not go away with the decision is the reason the row was ever held back. It is the only capability on this map with no parent in the three legal classes, and it repeats on a daily schedule the exact impression that answer 1 records as the starter credit's residual risk. Both facts are printed in I2 rather than resolved by the decision.
...
**All four were answered on 11 August 2026, at the stage 03a input gate, and the answers live in `docs/decisions.md` D-17.** The wording below is kept exactly as this map left it, because a map records the question it could not close and the answer is a different document's job. In short: `D-A` becomes two layers, a declaration at registration plus identity verification before funding, with the method inside layer 2 still open and the flow drawn as a superset. `D-B` goes to the live platform as a question of fact, and this map's fallback stands until it returns. `D-C` is scheduled before stage 04. `D-D` is deferred to 03b on purpose.
...
|---|---|---|---|---|---|
| M-01 | Locked rule | `:52` of `CLAUDE.md` against four rows here | "Provable fairness is the brand, not a footnote" against the secondary-proof framing, with the reconciliation still listing PF under "Confirmed" | Claude | Resolved by reframing. The feature is not demoted, one claim about it is. **Closed at step 11: design principle 1 in `CLAUDE.md` now carries the limit in its own text, so the rule file and this map say the same thing** |
| M-02 | Locked rule | design principle 3, `CLAUDE.md` | Expected value left the product. Step 7 dropped the only EV capability and nothing replaced it, while DANGER 3 at `jtbd.md:244` is open on exactly this | Claude | Fixed. Row D4 added and the T7 rejection annotated with the different grounds |
| M-03 | Locked rule | `CLAUDE.md`, compliance constraints | Staged regional rollout is a named first class compliance constraint and was absent from the file entirely | Claude | Fixed inside B4 |
...
| M-08 | Label mismatch | `jtbd.md:198` | The daily free case is inside the MVP free-to-play offer by founder Decision 3, which outranks a superseded canvas cell, and it was adjudicated only against `aarrr.md:172` | Claude, Codex | **Closed at step 11 by the founder, 11 August 2026: it enters round 1 as I2, with its missing parent stated in the row** |
| M-09 | Lost without a verdict | `CLAUDE.md`, Scope | Account is a round 1 item, had a group heading and no row, and F2 assumes an inventory exists | Claude | Fixed. Row B7 added |
